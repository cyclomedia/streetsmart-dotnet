...
namespace StreetSmart.Common.API
{
    ...
    public abstract class APIBase : IApiBase
    {
        ...
        protected readonly string _callFunctionBase;
        ...
        protected APIBase(IChromiumWebBrowserBase browser, string callFunctionBase)
          : this()
        {
            Browser = browser;
            _callFunctionBase = callFunctionBase;
        }
        ...
        public abstract void RegisterThisJsObject();

        public bool CheckResultTask(string funcName)
        {
            bool result = true;
        ...
        public string GetScript(string funcName, int processId = 0, [CallerMemberName] string memberName = "")
        {
            string memberId = $"{memberName}{processId}";
            return $"{JsThis}.{JsResult}({_callFunctionBase}.{funcName},{memberId.ToQuote()});";
        }

        public async Task<object> CallJsGetScriptAsync(string script, [CallerMemberName] string memberName = "")
        {
        ...
        public string AddTryCatch(string script, string funcName)
        {
            return $"try{{{script}}}catch(e){{{JsThis}.{JsStreetSmartException}(e.message,{funcName.ToQuote()});}}";
        }

        public virtual async Task<object> CallJsAsync(string script, int processId, [CallerMemberName] string memberName = "")
        {
            object funcResult = null;
            IBrowser myBrowser = !(Browser?.IsDisposed ?? true) ? GetBrowser() : null;
        ...
        protected abstract IBrowser GetBrowser();
